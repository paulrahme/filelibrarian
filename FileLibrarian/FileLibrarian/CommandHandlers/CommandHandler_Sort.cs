...
namespace FileLibrarian
{
    public class CommandHandler_Sort : CommandHandler
    {
        ...
        private class Comparer_FullPath : IComparer<FileInfoUtils>
        {
            public int Compare(FileInfoUtils x, FileInfoUtils y) => string.Compare(x.FileInfo.FullName, y.FileInfo.FullName);
        }

        private class Comparer_FileName : IComparer<FileInfoUtils>
        {
            public int Compare(FileInfoUtils x, FileInfoUtils y) => string.Compare(x.FileInfo.Name, y.FileInfo.Name);
        }

        private class Comparer_DirName : IComparer<FileInfoUtils>
        {
            public int Compare(FileInfoUtils x, FileInfoUtils y) => string.Compare(x.FileInfo.Directory.FullName, y.FileInfo.Directory.FullName);
        }

        private class Comparer_ModifiedTime : IComparer<FileInfoUtils>
        {
            public int Compare(FileInfoUtils x, FileInfoUtils y) => DateTime.Compare(x.FileInfo.LastWriteTime, y.FileInfo.LastWriteTime);
        }

        private class Comparer_FileSize : IComparer<FileInfoUtils>
        {
            public int Compare(FileInfoUtils x, FileInfoUtils y) => x.FileInfo.Length.CompareTo(y.FileInfo.Length);
        }
    }
}
